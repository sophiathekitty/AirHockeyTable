...
namespace IngameScript
{
    partial class Program
    {
        ...
        public class GridBlocks
        {
            ...
            public static IMyShipController GetPlayerLeft()
            {
                if (controllers.Count == 1) return controllers[0];
                return controllers.Find(x => x.CustomName.ToLower().Contains("left"));
            }
            public static IMyShipController GetPlayerRight()
            {
                if (controllers.Count == 1) return controllers[0];
                return controllers.Find(x => x.CustomName.ToLower().Contains("right"));
            }
